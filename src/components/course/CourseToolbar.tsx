    ...
    <div className="flex flex-col sm:flex-row justify-between items-center mb-6 gap-4">
      ...
      <div className="flex gap-2">
        <button
          ...
          className={`p-2 rounded-md ${view === 'grid' ? 'bg-orange-500 text-white' : 'bg-gray-100 text-gray-600 cursor-pointer'}`}
        >
          <Grid size={18} />
        </button>
        <button
          onClick={() => setView('list')}
          className={`p-2 rounded-md ${view === 'list' ? 'bg-orange-500 text-white' : 'bg-gray-100 text-gray-600 cursor-pointer'}`}
        >
          <List size={18} />
        </button>
      </div>
